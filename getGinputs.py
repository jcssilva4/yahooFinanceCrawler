...
for date in rebalanceDates[1:]: #the last date will not be included (only used to get forward returns) 
			...
			for line in filelines:
				...
				for asset in assetList:
					if(asset.code == neg_code):
						...
						#get expected return
						idx = idxRDates[idxrd]
						sampleMeanPrices = [] #sample mean prices
						for r in range(0,sampleDays):
							#print(sampleDates[idx])
							samplePrices = [] #sample prices
							idx2 = idx
							countSamplePrices = 0
							while (idx2 >= 0 and countSamplePrices <= 150):
								value = currAsset.DictPrices[sampleDates[idx2]].replace('\n','')
								if(not(value == '-')):
									samplePrices.append(float(value))
								countSamplePrices = countSamplePrices + 1
								idx2 = idx2 - 1
							if(len(samplePrices) > 0):
								samplePrices = np.array(samplePrices)
								sampleMeanPrices.append(np.mean(samplePrices))
							idx = idx - 1
							'''
							if(asset.code == 'ABEV3' and r == 0):
								print('idx2 = ' + str(idx2) + '\tpriceSample = ' + str(samplePrices))
							'''
						sampleSize = len(sampleMeanPrices)#used as an insertion criteria
						p1 =  currAsset.DictPrices[rebalanceDates[idxrd-1]].replace('\n','') #future price
						if(len(sampleMeanPrices)<sampleDays): #do not include this asset (more data needed!)
							#print(asset.DictReturns)
							notIncluded.append((date + '\t' + neg_code + '\t' + currAsset.name + '\n'))
						elif(p1 == '-'): #future price  = 0
							p1 = 0
						else: #include this asset
							observationsMatrix.append(sampleMeanPrices)
	...
	for r in range(0, sampleDays):
		idx3 =  idx
		sampleIdxPrices = [] #sample prices
		countSamplePrices = 0
		Rt = 0
		while (idx3 >= 0 and countSamplePrices <= 150):
			value = IdxDictPrice[sampleDates[idx3]]
			sampleIdxPrices.append(float(value))
			countSamplePrices = countSamplePrices + 1
			idx3 = idx3 - 1
		sampleIdxPrices = np.array(sampleIdxPrices)
		sampleMeanPrices.append(np.mean(sampleIdxPrices))
		idx = idx - 1
	Rt = 0
	for r in range(0, sampleDays-1):
		Rt = Rt + (sampleMeanPrices[r]/sampleMeanPrices[r+1]) - 1
	...
	for t in range(0,sampleDays-1):
		for i in range(0, len(assetvec)):
			...
			#print(Cplexvec)
		Cmat[t][len(assetvec)] = (sampleMeanPrices[t]/sampleMeanPrices[t+1]) - 1 #get index return
...
for date in rebalanceDates[1:]:
	#print(date)
	relative_path = "IbovespaData/rmat/rmat_" + date + ".txt"
	path = os.path.join(script_dir, relative_path)
	filemat = open(path,'w')
...
allIDX = 0 #use this to index ALLASSETS	
for date in rebalanceDates[1:]:
	relative_path = "IbovespaData/rmat/rvec_" + date + ".txt"
	path = os.path.join(script_dir, relative_path)
	filemat = open(path,'w')
	asset = ALLASSETS[allIDX]
	print(len(asset))
...
for date in rebalanceDates[1:]:
	#print(date)
	relative_path = "IbovespaData/rmat/cmat_" + date + ".txt"
	path = os.path.join(script_dir, relative_path)
	filemat = open(path,'w')
	asset = ALLASSETS[allIDX]
	print(len(asset))
...
relative_path = "IbovespaData/rmat/samplePeriodSize.txt"
path = os.path.join(script_dir, relative_path)
filemat = open(path,'w')
